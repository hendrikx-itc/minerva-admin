    ...
    match input {
        ...
        Ok(data) => {
            ...
            match result {
                Err(e) => HttpResponse::Conflict().body(e.to_string()),
                Ok(_) => {
                    let query_result = client.query_one("SELECT fm.id, m.id, dsrc_function, tsp.name, processing_delay::text, stability_delay::text, reprocessing_period::text, enabled, pg_proc.prosrc FROM trend_directory.function_materialization fm JOIN trend_directory.materialization m ON fm.materialization_id = m.id JOIN trend_directory.trend_store_part tsp ON st_trend_store_part_id = tsp.id JOIN pg_proc ON trend_directory.fingerprint_function_name(m) = proname WHERE tsp.name = $1", &[&data.target_trend_store_part],).await;
                    match query_result {
