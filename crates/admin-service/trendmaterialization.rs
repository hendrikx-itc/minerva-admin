...
pub(super) async fn get_trend_view_materializations(
    ...
    match clientquery {
        Err(e) => HttpResponse::InternalServerError().json(Error {
            code: 500,
            message: format!("Unable to interact with database: {}", e.to_string()),
        }),
        Ok(client) => {
