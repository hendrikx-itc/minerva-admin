...
pub struct InitializeOpt {
    ...
    #[structopt(long = "--with-definition", parse(from_os_str), help = "Minerva instance definition root directory")]
...
impl Cmd for InitializeOpt {
    async fn run(&self) -> CmdResult {
        ...
        if let Some(root) = minerva_instance_root {
            if !root.is_dir() {
                return Err(Error::Configuration(ConfigurationError::from_msg(format!("Not a valid directory: '{}'", &root.to_string_lossy()))));
            }
